...
uncomment if processing anew

```{r}
unnested_urls <- ss %>%
  select(-message) %>%
  unnest(links_list)
unnested_urls <- unnested_urls %>%
...
```{r}
# dealing with weird processing of some bitly links
my_long_urls_processed <- unnested_urls %>%
  ...
  select(-orig_url, -orig_url_second)
...
parsed_urls$url <- my_long_urls_processed$url
...
write_csv(dd, "comparing-links/data-for-shiny.csv")
...
dd %>% group_by(year, nces_id) %>% count(domain) %>% group_by(year) %>% mutate(prop = n / n()) %>% arrange(year, desc(n))
```
