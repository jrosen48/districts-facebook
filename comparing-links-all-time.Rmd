...
ss_collected <- tbl(cn, "posts") %>% 
  select(user_name, facebook_id, page_name, year)
collect()

d_ct_a <- read_rds("2019-2020-posts.rds") %>%
  mutate(user_name = tolower(user_name))
# 
# d_ct <- distinct(d, facebook_id, page_name, .keep_all = TRUE)
# 
# d_ct %>% 
...
```{r}
my_long_urls <- longurl::expand_urls(unnested_urls$links_list)
```
...
short_doms <- c("bit.ly", "ow.ly", "tinyurl", "buff.ly", "goo.gl", "tinyurl.com")
